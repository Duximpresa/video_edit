...
def text2speech(voice_txt_file, voice_name, project_name, speech_config):
    ...
    if not os.path.exists(voice_filename):
        ...
        try:
            print("【获取音频数据中】")
            result = synthesizer.speak_text_async(speechText).get()
            print("【获取音频数据成功】")
        except speechsdk:
            print("【获取音频数据失败】")
        try:
...
def main():
    ...
    text2speech(voice_file, voice_name, project_name, speech_config)
